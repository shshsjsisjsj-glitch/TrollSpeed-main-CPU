...
@implementation HUDRootViewController {
    ...
    ESP *_espView;
    imguiViewController *_imguiView;
#if !NO_TROLL
...
+ (instancetype)sharedInstance {
    static HUDRootViewController *sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[HUDRootViewController alloc] init];
        // 在这里进行初始化设置（如果需要的话）
    });
...
- (void)viewDidLoad
{
    ...
    _espView = [ESP sharedInstance];
    _espView.view.frame = _contentView.bounds;
    [self.view addSubview:_espView.view];
    
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//        _imguiView = [[imguiViewController alloc] init];
//        _imguiView.view.frame = _contentView.bounds;
//        [self.view addSubview:_imguiView.view];
    });
