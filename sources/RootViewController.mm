...
- (void)setHUDEnabled:(BOOL)enabled
{
    ...
    if (enabled && _localHUDRootViewController == nil) {
        _localHUDRootViewController = [HUDRootViewController sharedInstance];
...
- (void)loadView
{
    ...
    _ESPsettingsButton = [UIButton buttonWithType:UIButtonTypeSystem];
    [_ESPsettingsButton setTintColor:[UIColor whiteColor]];
    [_ESPsettingsButton addTarget:self action:@selector(ESPtapSettingsButton:) forControlEvents:UIControlEventTouchUpInside];
    [_ESPsettingsButton setImage:[UIImage systemImageNamed:@"gearshape"] forState:UIControlStateNormal];
    [self.backgroundView addSubview:_ESPsettingsButton];
    if (@available(iOS 15.0, *))
    {
        UIButtonConfiguration *config = [UIButtonConfiguration tintedButtonConfiguration];
        [config setCornerStyle:UIButtonConfigurationCornerStyleLarge];
        ...
        UIButtonConfiguration *config3 = [UIButtonConfiguration tintedButtonConfiguration];
        [config3 setCornerStyle:UIButtonConfigurationCornerStyleLarge];
        [_ESPsettingsButton setConfiguration:config3];
    }
    [_settingsButton setTranslatesAutoresizingMaskIntoConstraints:NO];
    [_ESPsettingsButton setTranslatesAutoresizingMaskIntoConstraints:NO];

    [NSLayoutConstraint activateConstraints:@[
        [_settingsButton.leadingAnchor constraintEqualToAnchor:self.view.leadingAnchor constant:60],
        [_settingsButton.bottomAnchor constraintEqualToAnchor:self.view.bottomAnchor constant:-50],
        [_ESPsettingsButton.trailingAnchor constraintEqualToAnchor:self.view.trailingAnchor constant:-60],
        [_ESPsettingsButton.bottomAnchor constraintEqualToAnchor:self.view.bottomAnchor constant:-50]
    ]];
...
- (void)ESPtapSettingsButton:(UIButton *)sender
{
    if (![_mainButton isEnabled]) return;
    log_debug(OS_LOG_DEFAULT, "- [RootViewController tapSettingsButton:%{public}@]", sender);

    ButtonViewController *settingsViewController = [[ButtonViewController alloc] init];
    [self presentViewController:settingsViewController animated:YES completion:nil];
}
